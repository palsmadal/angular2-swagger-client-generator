...
var Generator = (function () {
    ...
    function Generator(swaggerfile, outputpath, className, generate, modelInterfaces, fileName, modelPath, createModelPath, createModelExportFile, customTemplateFiles) {
        ...
        this._customTemplateFiles = customTemplateFiles;
    }
    ...
    Generator.prototype.initialize = function () {
        ...
        this.templates = {
            'interface': fs.readFileSync(this._customTemplateFiles.interface || __dirname + "/../templates/angular2-service.interface.mustache", 'utf-8'),
            'class': fs.readFileSync(this._customTemplateFiles.class || __dirname + "/../templates/angular2-service.mustache", 'utf-8'),
            'model': fs.readFileSync(this._customTemplateFiles.model || __dirname + "/../templates/angular2-model.mustache", 'utf-8'),
            'models_export': fs.readFileSync(this._customTemplateFiles.models_export || __dirname + "/../templates/angular2-models-export.mustache", 'utf-8')
        };
    ...
    Generator.prototype.createMustacheViewModel = function () {
        ...
        var data = {
            ...
            serviceFileName: this._fileName,
            ...
            _.forEach(api, function (op, m) {
                ...
                if (op.responses && op.responses['200'] && op.responses['200'].schema){
                    ...
                    if (schema.type) {
                        ...
                        // Do something here
                        //method.returns += '[]';
                        method.returns.isArray = true;
                        if (schema.items && schema.items['$ref']){
                            var refType = schema.items['$ref'];
                            method.returns.typescriptType = refType.substring(refType.lastIndexOf('/')+1);
                        }
                        } else {
                        method.returns.typescriptType = schema.type;
                        }
                    } else if (schema['$ref']){
                    ...
                    method.returns.typescriptType = 'void';
                }
                ...
                _.forEach(params, function (parameter) {
                    ...
                    parameter.camelCaseName = that.camelCase(parameter.name, true);
        ...
        _.forEach(swagger.definitions, function (defin, defVal) {
            ...
            var defineEnum = function(enumName, enumValues) {
                var exists = false;
    
                _.forEach(data.enums, function(elem) {
                    if (elem.name === enumName) {
                        exists = true;
                    }
                });
                ...
                if (!exists) {
                    ...
                    _.forEach(enumValues, function(elem) {
                        var name = that.camelCase(elem.toLowerCase());
                        var value = elem;
    
                        values.push({name: name, value: value});
                    });
    
                    definition.enums.push({ name: enumName, values: values });
                }
            }; 
            ...
            _.forEach(defin.properties, function (propin, propVal) {
                ...
                if (property.isArray)
                    ...
                    var enumName = that.camelCase(definition.name) + that.camelCase(property.name);
                    property.type = enumName;
                    defineEnum(enumName, propin.enum);
                }
                else
    ...
    Generator.prototype.camelCase = function (text, keepFirstCharCase) {
        if (!text)
            return text;

        if (text.indexOf('-') === -1 && text.indexOf('.') === -1 && text.indexOf('_') === -1) {
            if (keepFirstCharCase) {
                return text; 
            } else {
